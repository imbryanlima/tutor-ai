...
function de(e, t = {}) {
  ...
  for (; o.length; ) {
    ...
    for (let u in m2) {
      let g = m2[u], f = ve(g), p2 = z(u, a2) ? Q(d) : Q(d, ee(u));
...
var b2 = { regex: { rules: { class: { pattern: /^\.([a-zA-Z][\w-]*)$/, resolve(e) {
  ...
  var R, T, j, O, M, z2, V;
  let { preset: a2, options: n } = t, l, o, c, m2, d, u, g;
  ...
  return { primitive: { css: l, tokens: o }, semantic: { css: c, tokens: m2 }, global: { css: d, tokens: u }, style: g };
}, getPreset({ name: e = "", preset: t = {}, options: r, params: s3, set: i2, defaults: a2, selector: n }) {
  var f, x, p2;
  let l, o, c;
  if (s2(t) && r.transform !== "strict") {
    let y = e.replace("-directive", ""), m2 = t, { colorScheme: R, extend: T, css: j } = m2, O = v(m2, ["colorScheme", "extend", "css"]), d = T || {}, { colorScheme: M } = d, z2 = v(d, ["colorScheme"]), u = R || {}, { dark: V } = u, L = v(u, ["dark"]), g = M || {}, { dark: te } = g, re = v(g, ["dark"]), K = s2(O) ? this._toVariables({ [y]: h(h({}, O), z2) }, r) : {}, A2 = s2(L) ? this._toVariables({ [y]: h(h({}, L), re) }, r) : {}, X = s2(V) ? this._toVariables({ [y]: h(h({}, V), te) }, r) : {}, [G2, U2] = [(f = K.declarations) != null ? f : "", K.tokens || []], [B, I] = [(x = A2.declarations) != null ? x : "", A2.tokens || []], [H, W] = [(p2 = X.declarations) != null ? p2 : "", X.tokens || []], q2 = this.transformCSS(y, `${G2}${B}`, "light", "variable", r, i2, a2, n), Z = this.transformCSS(y, H, "dark", "variable", r, i2, a2, n);
  ...
  let a2 = function(l, o = {}, c = []) {
    ...
    Object.entries(l).forEach(([m2, d]) => {
      let u = z(m2, t.variable.excludedKeyRegex) ? o : o ? `${o}.${oe(m2)}` : oe(m2), g = c ? `${c}.${m2}` : m2;
      i(d) ? n(d, u, g) : (i2[u] || (i2[u] = { paths: [], computed: (f, x = {}, p2 = []) => {
  ...
  if (s2(t)) {
    ...
    if (s3 !== "style") {
      ...
      t = r === "dark" ? c.reduce((m2, { type: d, selector: u }) => (s2(u) && (m2 += u.includes("[CSS]") ? u.replace("[CSS]", t) : this.getSelectorRule(u, l, d, t)), m2), "") : C(l != null ? l : ":root,:host", t);
    }
    if (o) {
...
//# sourceMappingURL=chunk-UHKARXUA.js.map
