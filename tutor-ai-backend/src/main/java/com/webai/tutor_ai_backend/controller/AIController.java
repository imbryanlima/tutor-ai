...
public class AIController {
                ...
                try {
                        ...
                        UserProfile userProfile = userProfileRepository.findByUserId(currentUser.getId())
                                        .orElseThrow(() -> new RuntimeException(
                                                        "User Profile not found for user: " + userEmail));
                        String userLevel = userProfile.getEnglishLevel();

                        if (userLevel == null || userLevel.isBlank()) {
                                return ResponseEntity.badRequest()
                                                .body("Por favor, defina seu nível de inglês no perfil.");
                        }
                        ...
                        List<ChatMessage> recentHistory = chatHistoryFromDb
                                        .subList(Math.max(0, chatHistoryFromDb.size() - historyLimit),
                                                        chatHistoryFromDb.size());

                        List<MessageDTO> historyDTO = recentHistory.stream()
                                        .map(msg -> new MessageDTO(msg.getRole(), msg.getContent()))
                                        .collect(Collectors.toList());

                        String aiResponse = aiService.processGeneralMessage(userLevel, historyDTO,
                                        request.getMessage());
                        ...
                        logger.error("Erro inesperado ao processar a mensagem do usuário {}: {}",
        ...
        @GetMapping("/history")
        public ResponseEntity<List<MessageDTO>> getChatHistory(Authentication authentication) {
                try {
                        String userEmail = authentication.getName();
                        User currentUser = userRepository.findByEmail(userEmail)
                                        .orElseThrow(() -> new RuntimeException("User not found: " + userEmail));
                        ...
                        logger.error("Erro ao buscar histórico do usuário {}: {}", authentication.getName(),
                                        e.getMessage(), e);
